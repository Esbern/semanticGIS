...
import requests # Assuming you'll use the requests library

class ApiFetcherBase(ABC):
    """
    Abstract Base Class for API data fetchers.
    Defines a common interface for all specific API client implementations.
    """

    def __init__(self, api_key=None, base_url=None):
        self.api_key = api_key
        self.base_url = base_url
        self.session = requests.Session() # Common session for connection pooling, headers
        if self.api_key:
            self.session.headers.update({'Authorization': f'Bearer {self.api_key}'}) # Example auth
    ...
    def fetch_data(self, endpoint: str, params: dict = None) -> dict:
        ...
        """
        # No implementation here, or just a docstring.
        # Subclasses MUST provide their own.
        pass
    ...
    def get_service_status(self) -> str:
        ...
        Returns:
            str: A message indicating the status (e.g., "Operational", "Down").
        """
        pass

    def _make_request(self, method: str, url: str, params: dict = None, data: dict = None) -> requests.Response:
        """
        A helper method (not abstract) that concrete classes can use.
        It handles the actual HTTP request and basic error checking.
        """
        try:
    ...
    def some_common_utility(self):
        """
        An example of a concrete method in an ABC.
        Subclasses inherit this directly and can use it as is, or override it if needed.
        """
        print("This is a common utility method available to all fetchers.")
        return True

# --- Now, let's try to make a concrete class ---
